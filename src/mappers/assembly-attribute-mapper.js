...
export function extractAttributes(node, inputIndexed) {
    ...
    const attributesAggregatingModuleFeatures = extractAttributesAggregatingModuleFeatures(node, inputIndexed);
    const systemAttributes = extractSystemAttributes(node, inputIndexed);
    const attributesUsedInCases = extractAttributesUsedInCases(node, inputIndexed);
    const attributesControllingVariablePositions = extractAttributesControllingVariablePositions(node, inputIndexed);
    const technicalAttributes = generateTechnicalAttributes();
    const attributesAggregatingUpInAssemblyTree = generateAttributesAggregatingUpInAssemblyTree(node, inputIndexed);
...
function extractSystemAttributes(node, inputIndexed) {
...
function extractAttributesAggregatingModuleFeatures(node, inputIndexed) {
...
function extractAttributesUsedInCases(node, inputIndexed) {
...
function extractAttributesControllingVariablePositions(node, inputIndexed) {
...
function generateAttributesAggregatingUpInAssemblyTree(node, inputIndexed) {
 ...
 function groupAttributes(allAttributesWithDuplicates, inputIndexed) {
